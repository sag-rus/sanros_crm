...
function get_bnovo_rooms_prices($id_obj, $account_id, $dfrom, $dto, $id_rooms_bnovo = false) {
	...
	if (count($plans)>0 && count($rooms)>0) {
		$data = array(
			'token'  => $token,
			'account_id' => $account_id,
			'dfrom' => $dfrom,
		...
		foreach ($res['plans_data'] as $id_plan_bnovo => $roomdata) {
			foreach ($roomdata as $id_room_bnovo => $date_data) {
				foreach ($date_data as $date => $data) {
					...
					if($connect->getOne("SELECT COUNT(*) FROM ranges WHERE `id_obj`=$id_obj AND `type`=1 AND `place`='".$occus[$id_room_bnovo]['id_place']."' AND `id_date`=$id_date_price AND `rate_plan`='".$plans[$id_plan_bnovo]."'") == 0) {
						echo "SELECT COUNT(*) FROM ranges WHERE `id_obj`=$id_obj AND `type`=1 AND `place`='".$occus[$id_room_bnovo]['id_place']."' AND `id_date`=$id_date_price AND `rate_plan`='".$plans[$id_plan_bnovo]."'"."\r\n";
						$connect->query("INSERT INTO `ranges` SET `id`=0, `name`='from_bnovo', `id_obj`=$id_obj, `active`=0, `show_date`=1, `place`='".$occus[$id_room_bnovo]['id_place']."', `id_date`=$id_date_price, `counter`=1, `rate_plan`=".$plans[$id_plan_bnovo].", `treatment`=0");
						$id_range = $connect->insertId();
						echo "INSERT INTO `ranges` SET `id`=0, `name`='from_bnovo', `id_obj`=$id_obj, `active`=0, `show_date`=1, `place`='".$occus[$id_room_bnovo]['id_place']."', `id_date`=$id_date_price, `counter`=1, `rate_plan`=".$plans[$id_plan_bnovo].", `treatment`=0"."\r\n";
					} else {
					    $id_range = $connect->getOne("SELECT id FROM ranges WHERE `id_obj`=$id_obj AND `type`=1 AND `place`='".$occus[$id_room_bnovo]['id_place']."' AND `id_date`=$id_date_price AND `rate_plan`='".$plans[$id_plan_bnovo]."'");
					}
					echo 'id_range='.$id_range."\r\n";

					if($connect->getOne("SELECT COUNT(*) FROM price WHERE `id_room`=".$occus[$id_room_bnovo]['id_room']." AND `id_range`=$id_range") == 0) {
						echo "SELECT COUNT(*) FROM price WHERE `id_room`=".$occus[$id_room_bnovo]['id_room']." AND `id_range`=$id_range"."\r\n";
						$connect->query("INSERT INTO `price` SET `id`=0, `id_room`='".$occus[$id_room_bnovo]['id_room']."', `price`=".$data['price'].", `id_range`=$id_range, `active`=0, `date_last_save`='".date('H:i:s d.m.Y')."', `manager`='from bnovo'");
						$id_range = $connect->insertId();
						echo "INSERT INTO `price` SET `id`=0, `id_room`='".$occus[$id_room_bnovo]['id_room']."', `price`=".$data['price'].", `id_range`=$id_range, `active`=0, `date_last_save`='".date('H:i:s d.m.Y')."', `manager`='from bnovo'"."\r\n";
					} else {
					    $connect->query("UPDATE price SET `price`=?i WHERE `id_room`=".$occus[$id_room_bnovo]['id_room']." AND `id_range`=$id_range", $data['price']);
					}					
