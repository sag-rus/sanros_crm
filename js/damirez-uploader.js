(function($) {
	...
	jQuery.extend({
		...
		sendImageFileByFragments: function (start,fileNum,loaderID,fid,partnum,parts,uploadid) {
			...
			if(start >=0 && start< $.multUploderObjects[loaderID].files[fileNum]['file'].size) {
                ...
                $.multUploderObjects[loaderID].fragSent = $.ajax({
                    ...
                    success: function(data){
                        if(data['success']) {
                            if(typeof $prev.attr('data-upload-fid') == 'undefined') $prev.attr('data-upload-fid',data['fid']);
                            ...
                            if(data['loaded']) {
                                ...
                                var fids = {};
                                if($obj.val().length > 0) fids = JSON.parse($obj.val());
                                ...
                                $.sendImageFileByFragments(start+1*$uploader.attr('data-fragment-size'),fileNum,loaderID,data['fid'],data['partnum']+1,data['parts'],data['UploadId']);
                            }
                        }
                        else {
			...
			if($obj.hasClass('damirez-def-input-hide') && typeof $obj.attr('id') != 'undefined') {
				...
				for(fid in val) {
					if('uri' in val[fid]) {
						var bim = '', elSettings;
						if('mime' in val[fid]) {
							var typeFile = val[fid]['mime'].split('/')[0];
							if(typeFile == 'image')
								bim = 'background-image: url('+val[fid]['uri']+');';
						}
						if(!('title' in val[fid]))
							val[fid]['title'] = '';
						if(!('description' in val[fid]))
							val[fid]['description'] = '';
						...
						elSettings += '<input type="text" placeholder="Заголовок файла" class="element-title" value="'+val[fid]['title']+'"/>';
						elSettings += '</div>';
						elSettings +='<div class="input-block">';
						elSettings +='<div class="input-title">Описание</div>';
						elSettings += '<textarea class="element-description" placeholder="Описание">'+val[fid]['description']+'</textarea>';
						elSettings += '</div>';
						elSettings += '</div>';
						elSettings += '<div class="darkish-form-mask-buttons">';
						elSettings += '<button class="set-button darkish-form-mask-button elset-save-button">Закрыть</button>';
						elSettings += '</div>';
						elSettings +='</div>';
						elSettings += '<button class="set-button edit-button"><i class="fa fa-pencil-square-o fa-1" aria-hidden="true"></i></button>';
						$prevBlock.append('<div class="uploader-preview-element uploaded-el" style="'+bim+'" data-upload-fid="'+fid+'">'+elSettings+'<button class="set-button remove-button"></button><div class="upload-progress"><div class="upload-progress-rel"></div></div><a class="uploader-el-url" target="_blank" href="'+val[fid]['uri']+'">Ссылка</a></div>');
						fidAr[fid] = val[fid];
					}
				}
				$obj.val(JSON.stringify(fidAr));
			}
			else {
	...
	jQuery.fn.multUploader = function(options) {
		...
		var make = function(){
			...
			if(!$obj.hasClass('damirez-def-input-hide')) {
                ...
                $uploader.on('click','.uploader-preview-element .remove-button',function(e)
                {
                    ...
                    if(!$.multUploderObjects[loaderID].uploadState)
                    {
                        if($parEl.hasClass('uploaded-el')) {
                            var fids = JSON.parse($obj.val()+"");
                            delete fids[parseInt($parEl.attr('data-upload-fid'))];
                            $obj.val(JSON.stringify(fids))
                        }
                        $parEl.remove();
                    }
                ...
                $uploader.on('click','.uploader-preview-element .elset-save-button',function (e) {
                    var $but = $(this);
                ...
                $uploader.on('change','.uploader-preview-element input.element-title',function (e) {
                    ...
                    var $obj = $('#'+loaderID);
                    var fids = JSON.parse($obj.val()+"");
                    if(typeof $parEl.attr('data-upload-fid') != 'undefined') {
                        var fid = $parEl.attr('data-upload-fid');
                        if(fid in fids) {
                            fids[fid]['title'] = $titleInp.val();
                            $obj.val(JSON.stringify(fids));
                        }
                    }
                });
                $uploader.on('change','.uploader-preview-element textarea.element-description',function (e) {
                    ...
                    if(typeof $parEl.attr('data-upload-fid') != 'undefined') {
                        var fid = $parEl.attr('data-upload-fid');
                        if(fid in fids) {
                            fids[fid]['description'] = $descrInp.val();
                            $obj.val(JSON.stringify(fids));
                        }
                    }
                });
