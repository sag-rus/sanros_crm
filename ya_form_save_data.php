...
if ($exist_reckoning) {
	//$connect->query("INSERT 1_ya_form_log SET `id`=0, `datetime`=NOW(), `data`=?s", print_r($exist_reckoning, true));
	$connect->query(
	...
	//$connect->query("INSERT 1_ya_form_log SET `id`=0, `datetime`=NOW(), `data`=?s",$update_query);	
} else {
