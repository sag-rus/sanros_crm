...
$exist_reckoning = $connect->getRow("SELECT * FROM reckoning WHERE turist=?i AND status=1 AND id_obj=0 and `note`<>'' and `date`>NOW() - INTERVAL 1 DAY  ORDER BY id DESC LIMIT 1", $last_id);
if ($exist_reckoning) {
	$connect->query(
		"UPDATE reckoning 
		SET 
...
if ($dates_unavailable) {
	...
	if ($data_booking->adults > 0 && $data_booking->childs > 0 && $data_booking->tl=='1' && count($rest)<($data_booking->adults + $data_booking->childs)) {
		...
		file_put_contents('kostyl_booking.txt', PHP_EOL.'created child turist='.$child_last_id.' rest='.print_r($rest, true), FILE_APPEND);		
		if ($child_last_id>0) {
