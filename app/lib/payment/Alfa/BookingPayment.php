...
class BookingPayment {
  ...
  public function registerPayment($type = "", $qr = ''){
    ...
    if($sum > 0){
      ...
      if ($sum_to_pay==102000) {
        echo $url = $this->bankInfo['link'].'register.do?userName='.$this->bankInfo['userName'].'&password='.$this->bankInfo['password'].'&amount='.($sum_to_pay*100).'&currency=810';
        echo PHP_EOL;
        echo $url;
        exit();
      }
  ...
  public function depositPayment($bid_pay = ""){
    ...
    try {
      ...
      if ($_GET['debug']==1) echo '<pre>'.$log.'</pre>';
      file_put_contents('/var/www/html/CRM/alfalogs/alfa_deposit_log_'.date('Y-m-d').'.txt', date('d.m.Y H:i:s').' '.$log, FILE_APPEND);      
