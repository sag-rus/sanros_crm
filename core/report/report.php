  ...
  $data = $connect->getAll($zapros_for_mysql);
	...
	foreach($data as $row){
		...
		//if(($status == 4 AND $type_pay == 1) OR ($status == 5 AND $type_pay == 2) OR ($status_san == 3 AND $type_pay == 3) OR ($type_pay == 4) OR ($type_pay == 5)){
			...
			echo 'id='.$id.' $sum='.$row['sum'].' bank_com='.$row['bank_com'].'<br>';
			$sum_bank_kom = $row['sum'] * ($row['bank_com']/100);
			echo 'id='.$id.' $sum_bank_kom='.$sum_bank_kom.'<br>';
            $pay_reward = round(
				(
					(
			...
			echo 'id='.$id.' $pay_reward='.$pay_reward.'<br>';
