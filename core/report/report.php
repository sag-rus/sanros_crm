...
function filter_history_global($connect){
	...
	if($html) {
		...
		if ($user==42) {
			...
			if ($all['update_price_manager']==3989 && count($all['objects'])*20==1160) $all['update_price_manager'] = 4761;
			if ($all['update_price_manager']==3048 && count($all['objects'])*20==620) $all['update_price_manager'] = 3752;
			if ($all['update_price_manager']==3613 && count($all['objects'])*20==1120) $all['update_price_manager'] = 3786;
			if ($all['update_price_manager']==4811 && count($all['objects'])*20==1060) $all['update_price_manager'] = 5378;
			$stats .= '<br>';
			...
			$stats .= '<strong>Итого: </strong>'.(count($all['objects'])*20+$all['update_price_manager']*1+$all['foto_local']*3+$all['foto_url']*3).' руб. ( '.round((count($all['objects'])*20+$all['update_price_manager']*1+$all['foto_local']*3+$all['foto_url']*3)*1.15).' +15%)<br>';
		}
