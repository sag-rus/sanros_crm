...
function show_sites_list($connect) {
	...
	$sites = $connect->getAll("SELECT id, name, domain FROM sites WHERE status <> 2 ORDER BY id DESC");
	ob_start();
	?>
...
function save_sites_menu_item($connect) {
  ...
  $no_link = isset($_POST['no_link'])?(int)$_POST['no_link']:0;
  ...
  if((!$id || $menu_item) && (!$parent_id || $parent) && $site && $name && in_array($status,[0,1]) && in_array($main,[0,1]) && in_array($no_link,[0,1]) && in_array($menu_id,$menuArray) && ($href || $no_link)) {
    if($main) {
      if ($menu_item) {
    ...
    if($oldMenuItem) {
      ...
      $timestamp = gmdate("U");
      if($menu_item)
        $connect->query("UPDATE `app_models_site_menu_item` SET `changed`=?i, `name`=?s, `href`=?s, `main` = ?i, `menu_id` =?i, `status` =?i, `no_index` =?i, `no_link` =?i, `sort` =?i, `parent_id` = ?i WHERE `id` =?i",$timestamp, $name, $href, $main, $menu_id, $status, $no_index, $no_link,$sort,$parent_id,$menu_item['id']);
      else
        $connect->query("INSERT INTO `app_models_site_menu_item` (`created`, `changed`, `status`, `uid`, `sort`, `name`, `href`, `main`, `menu_id`, `site_id`, `parent_id`, `no_index`, `no_link`) VALUES (?i, ?i, ?i, ?i, ?i, ?s, ?s, ?i, ?i, ?i, ?i, ?i, ?i)",$timestamp, $timestamp, $status, 1, $sort, $name, $href, $main, $menu_id, $site['id'], $parent_id, $no_index, $no_link);
            ...
            <div class="modal-content">
                ...
                <div class="modal-body form-horizontal">
                  <?php if(($menu_item || $site_id) && ($parent || !$parent_id)) { ?>
                      ...
                      <div class="form-group">
                          <label class="col-sm-2 control-label">Без ссылки</label>
                          <div class="col-sm-10">
                              <input type="checkbox" name="no_link" class="form-control"<?php if($menu_item && $menu_item['no_link'] == 1) {?> checked<?php } ?>>
                          </div>
                      </div>
                  <?php } else { ?>
                    ...
                    <?php if($meta_template || $site_id) { ?>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                ...
                                <div class="info std-bottom-margin">
                                    <i class="fa fa-warning"></i> В случае пустоты значений использованных переменных шаблон будет проигнорирован для конкретной страницы!
                                </div>
                            </div>
                        </div>
...
function sites_content_author_options($connect, $selected_author_id = 0) {
  $selected_author_id = isset($_POST['selected_author_id'])?(int)$_POST['selected_author_id']:(int)$selected_author_id;
  $authors = $connect->getAll("SELECT `id`, `full_name` FROM `authors` WHERE `status` = 1 ORDER BY `full_name` ASC");
  ob_start();
  ?>
  <option value="0"<?php if(!$selected_author_id) { ?> selected<?php } ?>>Не выбран</option>
  <?php foreach ($authors as $author) { ?>
    <option value="<?=$author['id'];?>"<?php if($selected_author_id === (int)$author['id']) { ?> selected<?php } ?>><?=htmlspecialchars($author['full_name']);?></option>
  <?php }
  return ob_get_clean();
}
              ...
              <div class="modal-content">
                  ...
                  <div class="modal-body form-horizontal site-name">
	                      ...
	                      <div class="form-group">
	                          ...
	                          <div class="col-sm-10">
	                              <select class="form-control" name="author_id">
	                                  <?=sites_content_author_options($connect, $content['author_id']);?>
                              </select>
                              <div class="input-message-block" data-for="author_id"></div>
	                          </div>
	                      </div>
	                      <div class="form-group">
	                          <label class="col-sm-2 control-label">Редактор</label>
	                          <div class="col-sm-10">
	                              <select class="form-control" name="editor_id">
	                                  <?=sites_content_author_options($connect, $content['editor_id']);?>
	                              </select>
	                              <div class="input-message-block" data-for="editor_id"></div>
	                          </div>
	                      </div>
                      ...
                      <div class="form-group<?php if(trim($content['summary_cabinet'])=='') { ?> hidden<?php } ?>">
                          <label class="col-sm-2 control-label">Анонс<br>ИЗ КАБИНЕТА</label>
                          <div class="col-sm-10">
                              <textarea class="form-control" name="summary_cabinet"><?=htmlspecialchars($content['summary_cabinet']);?></textarea>
                              <label><input type="radio" name="summary_cabinet_accept" class="summary_cabinet_accept" value="1" <?php if ($content['summary_cabinet_accept']==2) echo 'disabled="disabled"';?>>принять</label> &nbsp; <label><input type="radio" name="summary_cabinet_accept" class="summary_cabinet_accept" value="2" <?php if ($content['summary_cabinet_accept']==2) echo 'disabled="disabled" checked="checked"';?>>отклонить</label> <input type="text" placeholder="укажите причину" name="summary_cabinet_not_accepted_reason" class="summary_cabinet_not_accepted_reason" <?php if ($content['summary_cabinet_accept']==2) echo 'disabled="disabled"';?> value="<?=$content['summary_cabinet_not_accepted_reason']?>"><br><br>
                          </div>
                      </div>                      
                      <div class="form-group<?php if(in_array($content['type'],['redirect'])) { ?> hidden<?php } ?>">
                      ...
                      <div class="form-group<?php if(trim($content['body_cabinet'])=='') { ?> hidden<?php } ?>">
                          <label class="col-sm-2 control-label">Содержимое<br>ИЗ КАБИНЕТА</label>
                          <div class="col-sm-10">
                              ...
                              <label><input type="radio" name="body_cabinet_accept" class="body_cabinet_accept" value="1" <?php if ($content['body_cabinet_accept']==2) echo 'disabled="disabled"';?>>принять</label> &nbsp; <label><input type="radio" name="body_cabinet_accept" class="body_cabinet_accept" value="2" <?php if ($content['body_cabinet_accept']==2) echo 'disabled="disabled" checked="checked"';?>>отклонить</label> <input type="text" placeholder="укажите причину" name="body_cabinet_not_accepted_reason" class="body_cabinet_not_accepted_reason" <?php if ($content['body_cabinet_accept']==2) echo 'disabled="disabled"';?> value="<?=$content['body_cabinet_not_accepted_reason']?>"><br><br>
                          </div>
                      </div>                      
                      <div class="form-group<?php if(!in_array($content['type'],['landing','settings', 'news', 'article', 'info', 'advice', 'blog_post'])) { ?> hidden<?php } ?>">
                      ...
                      <div class="form-group<?php if(trim($content['body2_cabinet'])=='') { ?> hidden<?php } ?>">
                          <label class="col-sm-2 control-label">Доп. содержимое<br>ИЗ КАБИНЕТА</label>
                          <div class="col-sm-10">
                              ...
                              <label><input type="radio" class="body2_cabinet_accept" name="body2_cabinet_accept" value="1" <?php if ($content['body2_cabinet_accept']==2) echo 'disabled="disabled"';?>>принять</label> &nbsp; <label><input type="radio" name="body2_cabinet_accept" class="body2_cabinet_accept" value="2" <?php if ($content['body2_cabinet_accept']==2) echo 'disabled="disabled" checked="checked"';?>>отклонить</label> <input type="text" placeholder="укажите причину" name="body2_cabinet_not_accepted_reason" class="body2_cabinet_not_accepted_reason" <?php if ($content['body2_cabinet_accept']==2) echo 'disabled="disabled"';?> value="<?=$content['body2_cabinet_not_accepted_reason']?>"><br><br>
                          </div>
                      </div>                      
                      ...
                      <div class="form-group">
                          <label class="col-sm-2 control-label">Запретить индексацию иозображений материала</label>
                          <div class="col-sm-10">
                              <input type="checkbox" name="imgs_no_index" class="form-control"<?php if($content['imgs_no_index'] == 1) {?> checked<?php } ?>>
                          </div>
                      </div>                      
                      <div class="form-group<?php if(!in_array($content['type'],['article','news','info', 'advice', 'blog_post'])) { ?> hidden<?php } ?>">
...
function set_sites_content($connect) {
  ...
  $search_links = isset($_POST['search_links'])?trim($_POST['search_links']):"";
  $site_id = isset($_POST['site_id'])?(int)$_POST['site_id']:0;
  ...
  $photogallery_orientations = ['album', 'book'];
	  ...
	  if($author_id > 0 && !$connect->getOne("SELECT `id` FROM `authors` WHERE `id` = ?i AND `status` = 1", $author_id)) {
	      $author_id = 0;
	  }
	  if($editor_id > 0 && !$connect->getOne("SELECT `id` FROM `authors` WHERE `id` = ?i AND `status` = 1", $editor_id)) {
	      $editor_id = 0;
	  }
  ...
  $status = isset($_POST['status'])?(int)$_POST['status']:0;
  $imgs_no_index = isset($_POST['imgs_no_index'])?(int)$_POST['imgs_no_index']:0;
          ...
          if(!$module_object_id || $connect->getOne("SELECT `id` FROM `object` WHERE `id` = ?i",$module_object_id)) {
            if($content_id) {
              ...
              if($content && $content['path'] !== $path && $type !== 'redirect' && !($type === 'aggregator' && $rss)) {
                  ...
                  if($site_id == 38) {
                    ...
                    if(count($contentPathExpl) > 2 && count($newPathExpl) > 2 && $contentPathExpl[1] === 'объект' && $newPathExpl[1] === 'направления') {
                      $connect->query("INSERT INTO `sites_contents` (`title`, `title_h1`, `title_h2`, `path`, `redirect_path`, `description`, `body`, `body2`, `summary`, `keywords`, `type`, `changed`, `published`, `status`, `imgs_no_index`, `synchronized`, `site_id`, `created`, `weight`, `sort`, `module_object_id`, `module_block`, `second_bg`, `form_action`, `map_code`, `landing_info`, `breadcrumb_title`, `photogallery_title`, `photogallery_orientation`, `direction_id`, `region_id`, `regional_direction_id`) VALUES (?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?i, ?i, ?i, ?i, ?i,  ?i, ?i, ?s, ?i, ?i, ?s, ?i, ?s, ?s, ?s, ?s, ?s, ?s, ?i, ?i, ?i)","Редирект", "", "", $content['path'].'/отзывы', $path.'/отзывы', "","","","","",'redirect',$timestamp,$published,1,$imgs_no_index,0,$content['site_id'],$timestamp,0.9, 0,0, '', 0, '','', '', '', '', 'album', 0, 0, 0);
                      $connect->query("INSERT INTO `sites_contents` (`title`, `title_h1`, `title_h2`, `path`, `redirect_path`, `description`, `body`, `body2`, `summary`, `keywords`, `type`, `changed`, `published`, `status`, `imgs_no_index`, `synchronized`, `site_id`, `created`, `weight`, `sort`, `module_object_id`, `module_block`, `second_bg`, `form_action`, `map_code`, `landing_info`, `breadcrumb_title`, `photogallery_title`, `photogallery_orientation`, `direction_id`, `region_id`, `regional_direction_id`) VALUES (?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?i, ?i, ?i, ?i, ?i, ?i, ?i, ?s, ?i, ?i, ?s, ?i, ?s, ?s, ?s, ?s, ?s, ?s, ?i, ?i, ?i)","Редирект", "", "", $content['path'].'/акции', $path, "","","","","",'redirect',$timestamp,$published,1,$imgs_no_index,0,$content['site_id'],$timestamp,0.9, 0,0, '', 0, '','', '', '', '', 'album', 0, 0, 0);
                      $connect->query("INSERT INTO `sites_contents` (`title`, `title_h1`, `title_h2`, `path`, `redirect_path`, `description`, `body`, `body2`, `summary`, `keywords`, `type`, `changed`, `published`, `status`, `imgs_no_index`, `synchronized`, `site_id`, `created`, `weight`, `sort`, `module_object_id`, `module_block`, `second_bg`, `form_action`, `map_code`, `landing_info`, `breadcrumb_title`, `photogallery_title`, `photogallery_orientation`, `direction_id`, `region_id`, `regional_direction_id`) VALUES (?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?i, ?i, ?i, ?i, ?i, ?i, ?i, ?s, ?i, ?i, ?s, ?i, ?s, ?s, ?s, ?s, ?s, ?s, ?i, ?i, ?i)","Редирект", "", "", $content['path'].'/описание', $path, "","","","","",'redirect',$timestamp,$published,1,$imgs_no_index,0,$content['site_id'],$timestamp,0.9, 0,0, '', 0, '','', '', '', '', 'album', 0, 0, 0);
                    }
              ...
              $respAr['msg'] = "Контент успешно добавлен";
	              $connect->query("INSERT INTO `sites_contents` (`title`, `slider_mode`, `title_h1`, `title_h2`, `path`, `redirect_path`, `description`, `body`, `body2`, `summary`, `snippet_summary`, `keywords`, `type`, `author_id`, `editor_id`, `changed`, `published`, `status`, `imgs_no_index`, `synchronized`, `site_id`, `created`, `weight`, `sort`, `module_object_id`, `module_block`, `second_bg`, `form_action`, `map_code`, `landing_info`, `breadcrumb_title`, `photogallery_title`, `photogallery_orientation`, `direction_id`, `region_id`, `regional_direction_id`, `rss`, `rss_aggregator_link`, `rss_addition`, `rss_aggregation`, `main_page_fix`, `aggregation_by_dates`, `aggregation_date_start`, `aggregation_date_end`, `head_code`, `pre_body_code`, `post_body_code`, `phone`) VALUES (?s, ?i, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?s, ?i, ?i, ?i, ?i, ?i, ?i, ?i, ?i, ?i, ?s, ?i, ?i, ?s, ?i, ?s, ?s, ?s, ?s, ?s, ?s, ?i, ?i, ?i, ?i, ?s, ?s, ?i, ?i, ?i, ?i, ?i, ?s, ?s, ?s, ?s)",$title, $slider_mode, $title_h1, $title_h2, $path, $redirect_path, $description, $body, $body2, $summary, $snippet_summary, $keywords, $type, $author_id, $editor_id, $timestamp, $published, $status, $imgs_no_index, 0, $site_id, $timestamp, $weight, $sort, $module_object_id, $module_block, $second_bg, $form_action, $map_code, $landing_info, $breadcrumb_title, $photogallery_title, $photogallery_orientation, $direction_id, $region_id, $regional_direction_id, $rss, $rss_aggregator_link, $rss_addition, $rss_aggregation, $main_page_fix, $aggregation_by_dates, $aggregation_date_start, $aggregation_date_end, $head_code, $pre_body_code, $post_body_code, $phone);

              $respAr['sql'] = $connect->last_query();
