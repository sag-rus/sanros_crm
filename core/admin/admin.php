...
?>
	<div class="form-horizontal panel panel-info">
		<div class="panel-heading pointer" role="button" data-toggle="collapse" data-parent="#change-cabinet" href="#object-<?= $id; ?>" aria-expanded="true" aria-controls="object-<?= $id; ?>">
			<i class="fa fa-comment-o"></i> Изменение описания объекта <?php echo $row["name"]; ?> <?php if ($row["status_datetime"]!='' ) echo ' (последнее изменение: '.date('d.m.Y H:m:s',strtotime($row["status_datetime"])).')'; ?>
		</div>
		...
		<div class="list-group">
			<div class="list-group-item list-hover-item">
				<div class="form-group form-group-margin">
					...
					<div class="col-sm-9">
						<?php echo date('d.m.Y H:m:s', strtotime($row["time"])); ?>
					</div>
				</div>
			</div>			
			<div class="list-group-item list-hover-item">
			...
			<div class="modal-body">
				<div class="form-horizontal edit-procedure">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" class="form-control postaddress" name="postaddress" value="<?php echo $row['postaddress']; ?>">
						</div>
					</div>					
					<div class="form-group">
...
function update_request_object($connect){
	...
	return json_encode(array("ok"=>1, "q"=>$q, 'POST'=>print_r($_POST, true)));
}
