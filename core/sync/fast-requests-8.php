...
if(file_exists($directory."/core/sync/file/fast-time-8.txt")) {
  $last_time = (int)file_get_contents($directory."/core/sync/file/fast-time-8.txt");
}

if(is_null($last_time) || file_exists($directory."/core/sync/file/fast-killing-time-8.txt")) {
  unlink($directory."/core/sync/file/fast-killing-time-8.txt");
  while(1) {
    file_put_contents($directory."/core/sync/file/fast-time-8.txt", time());
    if(!$connect) {
      file_put_contents($directory."/core/sync/file/fast-requests-error.log",'Database connection exception'.PHP_EOL,FILE_APPEND);
      unlink($directory."/core/sync/file/fast-time-8.txt");
      break;
    }
    ...
    try {
      $res = $client->request('POST',"https://sync2.tonia.ru/api/request/list/8".'?cache='.substr(str_shuffle('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'), 1, 15),[
        'form_params' => [
          'token' => $token
        ]
      ]);
    ...
    if(time() - $start_time > 56) {
      file_put_contents($directory."/core/sync/file/fast-killing-time-8.txt",(time()-$start_time));
      break;
    }
