...
echo '<pre>1config='.print_r($config, true).'</pre>';
echo '<pre>1configNew='.print_r($configNew, true).'</pre>';
...
echo '<pre>2config='.print_r($config, true).'</pre>';
...
echo '<pre>2configNew='.print_r($configNew, true).'</pre>';
...
if(file_exists($directory."/core/sync/file/fast-time-test.txt")) {
  $last_time = (int)file_get_contents($directory."/core/sync/file/fast-time-test.txt");
}

echo '123='.$last_time;

if (file_exists($directory."/core/sync/file/fast-killing-time-test.txt")) echo 'exist'; else echo 'NOT exist';

if(is_null($last_time) || file_exists($directory."/core/sync/file/fast-killing-time-test.txt")) {


  echo '456';

  unlink($directory."/core/sync/file/fast-killing-time-4.txt");
  while(1) {

    file_put_contents($directory."/core/sync/file/fast-time-test.txt", time());
    if(!$connect) {
          ...
          foreach ($res['requests'] as $id => $request) {
            ...
            if(function_exists($request['action'])) {
              try {
                ...
                echo '<pre>3config='.print_r($config, true).'</pre>';
                echo '<pre>3configNew='.print_r($configNew, true).'</pre>';
                ...
                echo '<pre>4config='.print_r($config, true).'</pre>';
                echo '<pre>4configNew='.print_r($configNew, true).'</pre>';
    ...
    if(time() - $start_time > 56) {
      file_put_contents($directory."/core/sync/file/fast-killing-time-4.txt",(time()-$start_time));
      break;
    }
