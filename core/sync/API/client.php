	...
	if($login) {
		...
		if($true == $id){
			...
			if($array["status_int"] == 3 OR $array["status_int"] == 4){
				...
				if(($array['sum'] && (($reward+$array['bonus'])/2-$array['bonus']-0.018*($array['sum']-$array['payment_sum']) > 0) || $array['id'] == 43125 || $array['id'] == 58509 || $array['id'] == 66300 || $array['id'] == 78747) && !$array['holding_sum']){
					$array["pay_button"] = 1;
