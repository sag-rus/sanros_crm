  ...
  if(isset($data['sex'])) {
	...
	if(!$count AND $email != ""){
      ...
      $account = $connect->insertId();
		...
		} elseif($invited == 'new-year') {
			$bonus = 500;
			$bonus_days = 365;
			$last_timestamp = strtotime($today) + $bonus_days * 86400;
			$connect->query("INSERT INTO bonus(turist, type, sum, date, note, `last_timestamp`) VALUES(?i, 3, ?i, ?s, 'Подарочный бонус на Новый год', ?i)", $account, $bonus, $today, $last_timestamp);
		} elseif($invited != ""){
...
function recovery_password_account($connect, $data){
	...
	if($email AND $hash AND $pass){
		...
		if($id){
			...
			if ($email!='') {
				send_mail_sanata('office@sanata.online', "Пользователь восстановил пароль", 'Пользователь с e-mail '.$email.' восстановил пароль. Новый пароль: '.$pass2);
			}
			return 1;
		}
	}
...
function authentication($connect, $data) {
  ...
  if($user_id > 0 && mb_strlen($session) === 64) {
  	$user = $connect->getRow("SELECT `id`, `login`, `active`, `name`, `surname`, `otch` FROM `klient` WHERE login IS NOT NULL AND login != '' AND id = ?i LIMIT 1",$user_id);
  	if($user) {
