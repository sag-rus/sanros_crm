...
if(file_exists($directory."/core/sync/file/fast-time-4.txt")) {
  $last_time = (int)file_get_contents($directory."/core/sync/file/fast-time-4.txt");
}

if(is_null($last_time) || file_exists($directory."/core/sync/file/fast-killing-time-4.txt")) {
  unlink($directory."/core/sync/file/fast-killing-time-4.txt");
  while(1) {
    file_put_contents($directory."/core/sync/file/fast-time-4.txt", time());
    if(!$connect) {
      file_put_contents($directory."/core/sync/file/fast-requests-error.log",'Database connection exception'.PHP_EOL,FILE_APPEND);
      unlink($directory."/core/sync/file/fast-time-4.txt");
      break;
    }
