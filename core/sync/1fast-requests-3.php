...
if(is_null($last_time) || file_exists($directory."/core/sync/file/fast-killing-time-3.txt")) {
  unlink($directory."/core/sync/file/fast-killing-time-3.txt");
  while(1) {
    ...
    if(!$connect) {
      ...
      unlink($directory."/core/sync/file/fast-time-3.txt");
      break;
    }
    ...
    try {
      $res = $client->request('POST',"https://sync2.tonia.ru/api/request/list/3".'?cache='.substr(str_shuffle('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'), 1, 15),[
        'form_params' => [
          'token' => $token
        ]
      ]);
    ...
    if(time() - $start_time > 56) {
      file_put_contents($directory."/core/sync/file/fast-killing-time-3.txt",(time()-$start_time));
      break;
    }
