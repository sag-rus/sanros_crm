...
if(is_null($last_time) || file_exists($directory."/core/sync/file/fast-killing-time-0.txt")) {
  ...
  while(1) {
    ...
    try {
      $res = $client->request('POST',"https://sync.tonia.ru/api/request/list/0".'?cache='.substr(str_shuffle('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'), 1, 15),[
        'form_params' => [
          'token' => $token
        ]
      ]);
