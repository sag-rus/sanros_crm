...
function review_contract($connect, $type, $id){
	...
	$object = get_object($connect, $id_obj, "full_and_address");
...
?>
	...
	<p>1.1. На условиях и в сроки, установленные настоящим Договором, ФИРМА обязуется реализовать санаторно-курортную путёвку, а КЛИЕНТ обязуется оплатить эту санаторно-курортную путёвку:<br /><br />
	...
	<p>2.1. ФИРМОЙ, являющимся непосредственным исполнителем бронирования санаторно-курортной путёвки, поименованной в п. 1.1 настоящего Договора, является следующее юридическое лицо:<br />
	...
	<!--ВНИМАНИЕ! КОГДА МЕНЯЮ ДАТЫ СТРАВАНИЯ ТУТ - МЕНЯТЬ ИХ И В КАБИНЕТЕ АГЕНСТВА!!! -->
	<strong>Размер финансового обеспечения:</strong> 500 000 рублей (Пятьсот тысяч рублей 00 копеек) до 28.02.2027 г.;<br />
	<strong>Номер, дата и срок действия договора страхования гражданской ответственности за неисполнение или ненадлежащее исполнение обязательств по договору о реализации санаторно-курортной путёвки:</strong><br />Договор страхования № СОТ 126737 от 11.11.2025, срок действия с 01.03.2026 по 28.02.2027 г.<br />
	<strong>Наименование, адрес (место нахождения) и почтовый адрес организации, предоставившей финансовое обеспечение:</strong><br /> АО "Страховая компания "Двадцать первый век", 191014, г. Санкт-Петербург, Литейный пр., д.57а, Лит. А.</p>
	...
	3.7. КЛИЕНТ обязан получить в установленное время и месте <?php if($id == 66100) { ?>санаторно-курортную путевку<?php } else { ?>турпутевку<?php } ?> и сопроводительные документы, заблаговременно согласовав с ФИРМОЙ место получения  документов, согласно п. 3.5 настоящего Договора.<br />
	3.8. ФИРМА обязана обеспечить предоставление всего комплекса услуг, входящих в санаторно-курортную путёвку, с надлежащим уровнем качества.<br />
	<?php if($id == 157862) { ?>
	3.9. В течение 14 (четырнадцати) рабочих дней с даты получения Заказчиком Акта приемки товаров, работ, услуг по форме 0510452(Приказ Минфина России от 28.06.2022 № 100н)(далее - Акт приемки), который формируется «Заказчиком» и предоставляется «Исполнителю», на основании документов счета-фактуры, акта выполненных работ и/или УПД, подтверждающих факт оказания услуг. «Исполнитель» в течение 2 (двух) рабочих дней после получения Акта приемки обязан подписать его и направить «Заказчику» по адресу: г. Октябрьский, ул. Девонская, д. 54а, каб. 213.
	<?php } ?>
	</p>
	...
	<p>4.1. Стоимость санаторно-курортной путёвки на дату заключения настоящего Договора, составляет <?php echo $arr[0]; ?> рублей <?php echo add_null($arr[1]); ?> копеек РФ
		...
		?>
		<?php if ($id!=125409) echo ', скидка составляет '.$sale.' рублей РФ ('.$itog_sum_sale.')'; else echo '.'; ?>
		<?php if ($id==125409) {echo ' Комиссия банка составляет 541.90 рублей РФ (пятьсот сорок один рубль 90 копеек). '; }?>
		<?php if ($id==125409) {echo ' Итого сумма к оплате '.$sum_pay.' рублей РФ ('.$itog_sum_pay.') '; } 
		  else echo 'Итого стоимость санаторно-курортной путёвки составляет '.$sum_pay.' рублей РФ ('.$itog_sum_pay.')'; ?>
		<?php if($id == 183565) {?>
			Отказ от поездки без фининсовых потерь до 03.06.2026  12:00 (мск). Аннуляция, начиная с указанной даты, повлечет за собой возмещение фактических затрат в размере 100% стоимости тура.
		<?php } ?> 
        <?php if($id == 79836) { ?>
            <br>&nbsp;4.1.1. В указанную стоимость входит:<br>
            &nbsp; &nbsp; Миронова Е. А. - 25 200 рублей (двадцать пять тысяч двести рублей 00 копеек)<br>
            &nbsp; &nbsp; Миронов С. Н. - 12 600 рублей (двадцать пять тысяч двести рублей 00 копеек)<br>
            &nbsp; &nbsp; Миронов Ф. Н. - 12 600 рублей (0 рублей 00 копеек)<br>
        <?php } elseif ($id == 79722) { ?>
            <br>&nbsp;4.1.1. В указанную стоимость входит:<br>
                Сагдеев А. Н. (основное взрослое место) - 26 950 рублей (двадцать шесть тысяч девятьсот пятьдесят рублей) 00 копеек<br>
                Сагдеева А. А. (основное взрослое место) - 26 950 рублей (двадцать шесть тысяч девятьсот пятьдесят рублей) 00 копеек<br>
                Сагдеев К. А. (долнительное детское место) - 14 752 рубля (четырнадцать тысяч семьсот пятьдесят два рубля) 50 копеек<br>
                Сагдеев А. А. (долнительное детское место) - 5 950 рублей (пять тысяч девятьсот пятьдесят рублей) 00 копеек<br>
        <?php } ?>
		<?php echo $prepay_str; ?>
		<br />
	<?php if ($id == 144998) {?>
		4.2 КЛИЕНТ одновременно с подписанием сторонами настоящего Договора вносит в счет оплаты стоимости санаторно-курортной путёвки в кассу или на расчетный счет ФИРМЫ аванс в размере 66000 рублей (Шестидесяти шести тысяч рублей) до 25 марта 2024 г. и остаток по счету  в размере 66000 рублей (Шестидесяти шести тысяч рублей) до 20 июня 2024 года.<br>
	<?php } else {?>
	...
	<?php } ?>
	4.3. При подтверждении бронирования <?php if($id == 66100) { ?>санаторно-курортной путевки<?php } else { ?>турпродукта<?php } ?> ФИРМОЙ КЛИЕНТ оплачивает в течение 5 рабочих дней с момента подтверждения 100% стоимости санаторно-курортной путёвки, с учетом ранее внесенных авансов.<br />
    ...
    5.4. В случае отказа КЛИЕНТА от исполнения договора, КЛИЕНТ оплачивает ФИРМЕ фактически понесенные последним расходы, связанные с исполнением обязательств по настоящему Договору.<br />
    ...
    5.7. В исключительных случаях ФИРМА может заменить заказанные и подтвержденные услуги с заменой объекта размещения и сохранением класса услуг. Такая замена не будет являться изменением существенных условий заказанных услуг. В случае несогласия КЛИЕНТОМ предложенных вариантов, ФИРМА производит полный возврат стоимости путевки.<br />
    5.8. В случае невозможности ФИРМОЙ выполнить взятые на себя обязательства, при отсутствии обстоятельств непреодолимой силы (ст. 401 ГК РФ), ФИРМА обязуется произвести возврат внесенных денежных средств КЛИЕНТА в течение 5-ти рабочих дней.
<!--
...
-->
	...
	6.3. Основанием для уплаты денежной суммы по договору страхования гражданской ответственности за неисполнение или ненадлежащее исполнение обязательств по договору о реализации санаторно-курортной путёвки является факт установления обязанности ФИРМЫ возместить КЛИЕНТУ реальный ущерб, возникший в результате неисполнения или ненадлежащего исполнения ФИРМОЙ указанных в п. 6.1 настоящего Договора обязательств, если это является существенным нарушением условий такого договора.<br />
