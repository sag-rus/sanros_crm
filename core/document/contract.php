...
?>
	...
	<strong>Размер финансового обеспечения на <?=date("Y");?> год:</strong> 500 000 рублей (Пятьсот тысяч рублей 00 копеек) до 28.02.2020 г.;<br />
	<strong>Номер, дата и срок действия договора страхования гражданской ответственности за неисполнение или ненадлежащее исполнение обязательств по договору о реализации санаторно-курортной путёвки:</strong><br />Договор страхования № 9646/18-49 от 15.11.2018, срок действия с 01.03.2019 по 28.02.2020 г.<br />
	...
	<p>4.1. Стоимость санаторно-курортной путёвки на дату заключения настоящего Договора, составляет <?php echo $arr[0]; ?> рублей <?php echo add_null($arr[1]); ?> копеек РФ
        ...
        <?php } elseif ($id == 79722) { ?>
            <br>&nbsp;4.1.1. В указанную стоимость входит:<br>
                ...
                Сагдеев К. А. (долнительное детское место) - 14 752 рубля 50 копеек (четырнадцать тысяч семьсот пятьдесят два рубля 50 копеек)<br>
