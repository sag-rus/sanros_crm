...
function review_schet($connect, $type = "PDF", $id, $for = ""){
	...
	foreach($data as $row){
		...
		if($id_room != 0)
			$table.= "<td width='450'>".$object." c ".$date_z." по ".$date_v." (".$days." дн., ".$room.") ".$note."</td>";
		else
			$table.= "<td width='450'>".$connect->getOne("SELECT name FROM service_schet WHERE id=?i", $id_service)." ".$note."</td>";
