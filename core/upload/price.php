...
function upload_price_on_server($connect, $id=false, $nthChild = NULL,$showProccess = false){
  ...
  if(!$id) {
    ...
    $server_file = "/price/XML/price/".$id.".xml";
    ...
    if(ftp_nlist($connect_server,"/price/json/price") == false) {
      @ftp_mkdir($connect_server, "/price/json/price");
      @ftp_chmod($connect_server,0777,"/price/json/price");
    }

		//$server_file = "/var/www/default-site/public_html/price/json/price/".$id.".json";
    $server_file = "/price/json/price/".$id.".json";
		if(!ftp_put($connect_server, $server_file, $file, FTP_ASCII))
			return "Не удалось загрузить файл на сервер";
		@ftp_chmod($connect_server, 0777, $server_file);
    ...
    $server_file = "/price/json/price/".$id.".cache";
		if(!ftp_put($connect_server, $server_file, $file, FTP_ASCII))
			return "Не удалось загрузить файл на сервер";
		@ftp_chmod($connect_server, 0777, $server_file);
    ...
    $server_file = "/price/XML/desc/".$id.".xml";
		if(!ftp_put($connect_server, $server_file, $file, FTP_ASCII))
			return "Не удалось загрузить файл на сервер";
		@ftp_chmod($connect_server, 0777, $server_file);
		if($showProccess)
